...
def load_model_and_vectorizer(model_name, model_version, vectorizer_path):
    ...
    try:
        print("sklearn model")
        model_uri = f"models:/{model_name}/{model_version}"
        model = mlflow.sklearn.load_model(model_uri)  # Using sklearn loader instead of pyfunc  
    except:
        # Fallback to pyfunc if sklearn doesn't work
        model = mlflow.pyfunc.load_model(model_uri) # pyfunc will not work
    vectorizer = joblib.load(vectorizer_path)  # Load the vectorizer
    print(f"Vectorizer vocabulary size: {len(vectorizer.vocabulary_)}")
    print(f"Vectorizer feature names: {len(vectorizer.get_feature_names_out())}")
...
def predict():
    ...
    try:
        ...
        # logger.debug(preprocessed_comments)
        transformed_comments = vectorizer.transform(preprocessed_comments)
        # logger.debug("After vectorizer")
        # logger.debug(transformed_comments)
        # Make predictions
